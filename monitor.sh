...
##### optional:        #
identityPubkey="SDEVqCDyc3YzjrDn375SMWKpZo1m7tbZ12fsenF48x1"      # identity pubkey for the validator, insert if autodiscovery fails
voteAccount="9jYFwBfbjYmvasFbJyES9apLJDTkwtbgSDRWanHEvcRw"         # vote account address for the validator, specify if there are more than one or if autodiscovery fails
...
binDir="/mnt/jito-solana/target/release/solana-validator"              # auto detection of the solana binary directory can fail or an alternative custom installation is preferred, in case insert like $HOME/solana/target/release
